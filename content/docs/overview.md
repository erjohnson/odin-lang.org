...
We'll start with a modified version of the famous "hello world" program. Create a file called `hellope.odin` with the following contents:
...
Use the `odin run` command to compile and run the executable:

```
odin run hellope.odin
```

Or use the `odin build` command to compile the executable
...
The Odin compiler parses comments as tokens. This will enable future work on automatic documentation tools.
...
Find the length of a string with the built-in `len` proc:
...
Constants are evaluated at compile time and their assigned value cannot be changed.
...
Add the `private` attribute to an entity to prevent it from being exported from a package.
...
Loops stop executing when the condition evaluates to `false`.
...
Omit the condition to create an infinite loop:
...
A second index value can be added:
...
Iterated values are *copies* and cannot be written to. The following idiom is useful for iterating over a container in a by-reference manner:
...
**Note:** Odin's `defer` differs from Go's `defer`, which is function-exit and relies on a closure stack system.
...
Use a `fallthrough` statement to explicitly fall through into the next case block:
...
In Odin, a procedure is something that can do work. In other languages these are called _functions_ or _methods_. Procedures are defined with the `proc` keyword:
...
When two or more consecutive parameters share a type, you can omit the other types from previous names, like with variable declarations. In the above example: `x: int, y: int` can be shortened to `x, y: int`:
...
Everything in Odin is passed by value. Procedures always get a copy of the thing passed to them, as if there was an assignment statement to the procedure parameter.

Passing a pointer value makes a copy of the pointer, not the data it points to. Slices, dynamic arrays, and maps behave like pointers in this case. Internally, they are structures with values, which include pointers, and the "structure" is passed by value.

Parameters in a procedure body are mutable, but since they are copies, they will not affect the original values.
...
Return values in Odin may be named. Named results are treated as variables defined at the top of the procedure, like input parameters.
...
A `return` statement without arguments returns the named return value. "Naked" return statements should only be used in short procedures as it reduces clarity when reading.
...
Procedure arguments can be named to make it clear which argument a parameter is for:
...
Procedure arguments can be assigned default values. Default values must be compile time known values, such as a constant value or `nil` (if the type supports it).

The previous `create_window` procedure may be easier to use if default values are provided. The default values will be used if they are not specified:
...
Append new elements to a dynamic array with the built-in `append` proc.
